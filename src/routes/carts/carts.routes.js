...
router.post('/:cid/product/:pid', async (req, res) => {
  ...
  try {
    ...
    const prodAdd = await cartManagerMongo.addProductCart(cid, pid, quantity)
    console.log(prodAdd)
    return res.status(200).json({
  ...
  try {
    ...
    for (const prod of products) {
      if (
        ...
        return res.status(400).json({
          message: 'New products must comply with the specified format',
        })
      }
...
router.put('/:cid/products/:pid', async (req, res) => {
  ...
  try {
    ...
    const quantityAdd = await cartManagerMongo.putQuantityProduct(
      cid,
      pid,
      quantity
    )
...
export const baseUrlCarts = `${baseUrl}carts/`
